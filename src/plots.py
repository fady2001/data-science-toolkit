"""
Data visualization and exploratory data analysis plotting utilities.

This module provides comprehensive plotting functions for analyzing the Titanic dataset,
including survival analysis, demographic analysis, correlation studies, fare analysis,
and missing data visualization. It uses matplotlib and seaborn for creating publication-ready plots.
"""
...
def plot_correlation_analysis(df: pd.DataFrame, output_dir: Path):
    ...
    sns.heatmap(correlation_matrix, annot=True, cmap="coolwarm", center=0, square=True, ax=axes[0])
    axes[0].set_title("Feature Correlation Matrix")

    # Survival correlation
    survival_corr = (
        correlation_matrix["Survived"].drop("Survived").sort_values(key=abs, ascending=False)
    )
    survival_corr.plot(
        kind="bar", ax=axes[1], color=["red" if x < 0 else "green" for x in survival_corr]
    )
...
def plot_survival_rates(df: pd.DataFrame, output_dir: Path):
    ...
    for i, (idx, row) in enumerate(family_survival.iterrows()):
        axes[1, 0].text(
            i, row["mean"] + 0.01, f"n={row['count']}", ha="center", va="bottom", fontsize=8
        )
...
def plot_fare_analysis(df: pd.DataFrame, output_dir: Path):
    ...
    axes[1, 0].scatter(
        died_passengers["Age"], died_passengers["Fare"], alpha=0.6, c="red", label="Died", s=20
    )
    axes[1, 0].scatter(
...
def main(cfg: DictConfig):
    ...
    figures_dir = (
        proj_root / cfg["paths"]["reports_parent_dir"] / cfg["paths"]["figures_parent_dir"]
    )
